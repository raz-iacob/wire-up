@php
    $current = fn (string $name): bool => app('livewire')?->isLivewireRequest()
        ? str()->is(mb_trim(str(route($name))->after(config('app.url'))->toString(), '/') ?: '/', app('livewire')->originalPath())
...
<div class="space-y-8">
    <flux:tabs variant="pills" scrollable>
        <flux:tab
            ...
            :selected="$current('admin.account-profile')"
            wire:navigate
        >{{ __('Profile') }}</flux:tab>
        <flux:tab
            :href="route('admin.account-password')"
            :selected="$current('admin.account-password')"
            wire:navigate
