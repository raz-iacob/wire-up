    ...
    <div class="flex flex-col gap-2">
                ...
                >
                    <div class="flex items-center justify-between gap-3 bg-zinc-100 px-3 py-2 dark:bg-white/10">
                        ...
                        >
                            <flux:heading
                                size="sm"
                                class="truncate"
                            >{{ $planName !== '' ? $planName : __('Plan :number', ['number' => $i + 1]) }}</flux:heading>
                        </button>
                        ...
                        <div wire:sort:ignore class="flex shrink-0 items-center gap-1">
                            ...
                            <flux:button
                                ...
                                icon="trash"
                                square
                                wire:click="removePricingItem('{{ $block['id'] }}', {{ $i }})"
                                :tooltip="__('Remove plan')"
                            />
                        </div>
                    </div>
                    ...
                    <div class="flex flex-col gap-4 p-4" x-show="open" x-collapse x-cloak>
                        ...
                        <div class="grid gap-4 md:grid-cols-2">
                            <x-forms.input-translated
                                name="{{ $c }}.items.{{ $i }}.price"
                                :locale="$locale"
                                :multi-locale="$multiLocale"
                                label="{{ __('Price') }}"
                            />
                            <x-forms.input-translated
                                name="{{ $c }}.items.{{ $i }}.period"
                                :locale="$locale"
                                :multi-locale="$multiLocale"
                                label="{{ __('Period') }}"
                            />
                        ...
                        <flux:switch
                            wire:model.live="{{ $c }}.items.{{ $i }}.featured"
                            label="{{ __('Highlight this plan') }}"
                            align="left"
                        />

                        <div x-show="{{ $b }}?.items?.[{{ $i }}]?.featured" x-cloak>
                            <x-forms.input-translated
                                name="{{ $c }}.items.{{ $i }}.badge"
                                :locale="$locale"
                                :multi-locale="$multiLocale"
                        ...
                        <flux:switch
                            wire:model.live="{{ $c }}.items.{{ $i }}.cta.enabled"
                            label="{{ __('Show a button') }}"
                            align="left"
                        />

                        <div x-show="{{ $b }}?.items?.[{{ $i }}]?.cta?.enabled" class="grid gap-4 md:grid-cols-2">
                            <x-forms.input-translated
                                name="{{ $c }}.items.{{ $i }}.cta.text"
                                :locale="$locale"
                                :multi-locale="$multiLocale"
                            ...
                            <flux:select
                                wire:model.live="{{ $c }}.items.{{ $i }}.cta.link.type"
                                variant="listbox"
                                label="{{ __('Link to') }}"
                            >
                            ...
                            <div class="col-span-2">
                                @if ($linkType === 'page')
                                    <flux:select
                                        wire:model="{{ $c }}.items.{{ $i }}.cta.link.value"
                                        variant="listbox"
                                        searchable
                                    ...
                                    >
                                        @foreach ($pageOptions as $pageId => $pageTitle)
                                            <flux:select.option value="{{ $pageId }}">
                                                {{ $pageTitle }}</flux:select.option>
                                        @endforeach
                                    </flux:select>
                                @elseif ($linkType === 'anchor')
                                    <flux:input
                                        wire:model.lazy="{{ $c }}.items.{{ $i }}.cta.link.value"
                                ...
                                @else
                                    <div class="flex flex-col gap-3">
                                        ...
                                        />
                                        <flux:switch
                                            wire:model.lazy="{{ $c }}.items.{{ $i }}.cta.link.newTab"
                            ...
                            <flux:color-picker
                                wire:model="{{ $c }}.items.{{ $i }}.cta.bg"
                                clearable
                                label="{{ __('Button color') }}"
                                placeholder="{{ __('Theme') }}"
                            />
                            <flux:color-picker
                                wire:model="{{ $c }}.items.{{ $i }}.cta.textColor"
                                clearable
                                label="{{ __('Text color') }}"
                                placeholder="{{ __('Theme') }}"
                            />
        ...
        <div>
            <flux:button
                size="sm"
                icon="plus"
                wire:click="addPricingItem('{{ $block['id'] }}')"
            >{{ __('Add plan') }}</flux:button>
        </div>
    </div>
