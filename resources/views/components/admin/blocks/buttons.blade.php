                    ...
                    <div class="flex items-center justify-between gap-3 bg-zinc-100 px-3 py-2 dark:bg-white/10">
                        ...
                        >
                            <flux:heading
                                size="sm"
                                class="truncate"
                            >{{ $buttonTitle !== '' ? $buttonTitle : __('Button :number', ['number' => $i + 1]) }}</flux:heading>
                        </button>
                        ...
                        <div wire:sort:ignore class="flex shrink-0 items-center gap-1">
                            ...
                            <flux:button
                                ...
                                icon="trash"
                                square
                                wire:click="removeButtonItem('{{ $block['id'] }}', {{ $i }})"
                                :tooltip="__('Remove button')"
                            />
                        </div>
                    </div>

                    <div class="grid gap-4 p-4 md:grid-cols-2" x-show="open" x-collapse x-cloak>
                        ...
                        <flux:select
                            wire:model.live="{{ $c }}.items.{{ $i }}.variant"
                            variant="listbox"
                            label="{{ __('Style') }}"
                        >
                        ...
                        <flux:select
                            wire:model.live="{{ $c }}.items.{{ $i }}.link.type"
                            variant="listbox"
                            label="{{ __('Link to') }}"
                        >
                        ...
                        <div class="md:col-span-2">
                            @if ($linkType === 'page')
                                <flux:select
                                    wire:model="{{ $c }}.items.{{ $i }}.link.value"
                                    variant="listbox"
                                    searchable
                            ...
                            @elseif ($linkType === 'anchor')
                                <flux:input
                                    wire:model.lazy="{{ $c }}.items.{{ $i }}.link.value"
                            ...
                            @else
                                <div class="flex flex-col gap-3">
                                    ...
                                    />
                                    <flux:switch
                                        wire:model.lazy="{{ $c }}.items.{{ $i }}.link.newTab"
        ...
        @if (count($items) < 3)
            <div>
                <flux:button
                    size="sm"
                    icon="plus"
                    wire:click="addButtonItem('{{ $block['id'] }}')"
                >{{ __('Add button') }}</flux:button>
            </div>
        @endif
    </div>
