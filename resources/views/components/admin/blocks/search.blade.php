...
<div class="flex flex-col gap-6">
    ...
    <flux:pillbox
        ...
        multiple
        searchable
        label="{{ __('Content types to search') }}"
        placeholder="{{ __('Choose what visitors can search…') }}"
    >
        @foreach ($recordTypes as $recordType)
    ...
    <x-forms.input-translated
        ...
        :locale="$locale"
        :multi-locale="$multiLocale"
        label="{{ __('Search box placeholder') }}"
    />
    ...
    <flux:input
        ...
        min="1"
        max="24"
        wire:model.live="{{ $c }}.perType"
        label="{{ __('Results shown per type') }}"
        description="{{ __('The total match count is always shown; this limits how many cards appear under each type.') }}"
    />
