...
<div class="flex flex-col gap-6">
    <flux:input
        wire:model.lazy="{{ $c }}.formName"
        label="{{ __('Form name') }}"
        placeholder="{{ __('e.g. Massage enquiry') }}"
        description="{{ __('Shown in the notification email so you know which form was submitted.') }}"
    />
    ...
    <div class="flex flex-col gap-2">
        ...
        <div
            wire:sort="reorderContactFields"
            wire:sort:group="contact-fields-{{ $block['id'] }}"
            ...
            @foreach ($fieldOrder as $token)
                        ...
                        <div class="flex items-center justify-between gap-3 bg-zinc-100 px-3 py-2 dark:bg-white/10">
                            ...
                            <div wire:sort:ignore class="flex shrink-0 items-center gap-1">
                                ...
                                <flux:button
                                    ...
                                    icon="trash"
                                    square
                                    wire:click="removeContactField('{{ $block['id'] }}', '{{ $token }}')"
                                    :tooltip="__('Remove field')"
                                />
                            </div>
                        </div>
                ...
                @else
                    ...
                    <flux:card
                        ...
                        wire:key="cf-{{ $token }}"
                        wire:sort:item="{{ $block['id'] }}::{{ $token }}"
                        x-data="{ open: window.claimsOpenBlockItem('{{ $token }}') }"
                    ...
                    >
                        <div class="flex items-center justify-between gap-3 bg-zinc-100 px-3 py-2 dark:bg-white/10">
                            ...
                            >
                                <flux:heading
                                    size="sm"
                                    class="truncate"
                                >{{ $fieldName !== '' ? $fieldName : __('Custom field') }}</flux:heading>
                            </button>
                        ...
                        <div class="flex flex-col gap-4 p-4" x-show="open" x-collapse x-cloak>
                            ...
                            <flux:select
                                wire:model.live="{{ $c }}.customFields.{{ $i }}.type"
                                variant="listbox"
                                label="{{ __('Type') }}"
                            >
                            ...
                            <div x-show="{{ $b }}?.customFields?.[{{ $i }}]?.type === 'select'" x-cloak>
                                <flux:textarea
                                    wire:model.lazy="{{ $c }}.customFields.{{ $i }}.options"
                                    label="{{ __('Options') }}"
                                    rows="4"
                            ...
                            <flux:switch
                                wire:model.live="{{ $c }}.customFields.{{ $i }}.required"
                                label="{{ __('Required') }}"
                                align="left"
                            />
                            ...
                            <div x-show="{{ $b }}?.layout === 'split'" x-cloak>
                                <flux:radio.group
                                    wire:model.live="{{ $c }}.customFields.{{ $i }}.column"
                                    variant="segmented"
                                    size="sm"
                                    label="{{ __('Column') }}"
                                >
            ...
            <flux:dropdown position="bottom" align="start">
                ...
                <flux:menu>
                    @foreach ($missingBuiltins as $key)
                        <flux:menu.item wire:click="addContactBuiltin('{{ $block['id'] }}', '{{ $key }}')">
                            {{ $builtins[$key] }}</flux:menu.item>
                    @endforeach
                    @if ($missingBuiltins !== [])
                        <flux:menu.separator />
                    @endif
                    <flux:menu.item wire:click="addContactField('{{ $block['id'] }}')">
                        {{ __('Custom field') }}</flux:menu.item>
                </flux:menu>
            </flux:dropdown>
        </div>
    </div>
    ...
    <flux:input
        wire:model.lazy="{{ $c }}.recipient"
        label="{{ __('Send submissions to') }}"
        placeholder="{{ __('Defaults to your site contact email') }}"
