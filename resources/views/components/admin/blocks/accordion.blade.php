...
<div class="flex flex-col gap-6">
    ...
    <div class="flex flex-col gap-2">
        ...
        <div
            wire:sort="reorderAccordionItems"
            wire:sort:group="accordion-{{ $block['id'] }}"
                    ...
                    <div class="flex items-center justify-between gap-3 bg-zinc-100 px-3 py-2 dark:bg-white/10">
                        ...
                        >
                            <flux:heading
                                size="sm"
                                class="truncate"
                            >{{ $itemTitle !== '' ? $itemTitle : __('Item :number', ['number' => $i + 1]) }}</flux:heading>
                        </button>
                        ...
                        <div wire:sort:ignore class="flex shrink-0 items-center gap-1">
                            ...
                            <flux:button
                                ...
                                icon="trash"
                                square
                                wire:click="removeAccordionItem('{{ $block['id'] }}', {{ $i }})"
        ...
        <div>
            <flux:button
                size="sm"
                icon="plus"
                wire:click="addAccordionItem('{{ $block['id'] }}')"
            >{{ __('Add item') }}</flux:button>
        </div>
    </div>
    ...
    <div class="flex flex-col gap-4">
        <flux:switch
            wire:model.lazy="{{ $c }}.exclusive"
            label="{{ __('Only one item open at a time') }}"
            align="left"
        />
