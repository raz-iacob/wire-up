...
<div class="flex flex-col gap-6">
    ...
    <div class="grid gap-4 sm:grid-cols-2">
        <flux:input
            wire:model.lazy="{{ $c }}.phone"
            type="tel"
            label="{{ __('Phone') }}"
            placeholder="+1 555 123 4567"
        />
        <flux:input
            wire:model.lazy="{{ $c }}.email"
            type="email"
            label="{{ __('Email') }}"
    ...
    <div class="flex flex-col gap-4">
        ...
        <flux:switch
            ...
            label="{{ __('Display map on the right') }}"
            align="left"
        />
    </div>
    ...
    <div x-show="{{ $b }}?.directions?.enabled" class="grid gap-4 md:grid-cols-2">
        <x-forms.input-translated
            name="{{ $c }}.directions.text"
            :locale="$locale"
            :multi-locale="$multiLocale"
        ...
        <div class="grid grid-cols-2 gap-4">
            <flux:color-picker
                wire:model="{{ $c }}.directions.bg"
                clearable
                label="{{ __('Button color') }}"
                placeholder="{{ __('Theme') }}"
            />
            <flux:color-picker
                wire:model="{{ $c }}.directions.textColor"
                clearable
                label="{{ __('Text color') }}"
                placeholder="{{ __('Theme') }}"
            />
