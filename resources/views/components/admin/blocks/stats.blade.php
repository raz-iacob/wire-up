    ...
    <div class="flex flex-col gap-2">
                ...
                >
                    <div class="flex items-center justify-between gap-3 bg-zinc-100 px-3 py-2 dark:bg-white/10">
                        ...
                        >
                            <flux:heading
                                size="sm"
                                class="truncate"
                            >{{ $statTitle !== '' ? $statTitle : __('Stat :number', ['number' => $i + 1]) }}</flux:heading>
                        </button>
                        ...
                        <div wire:sort:ignore class="flex shrink-0 items-center gap-1">
                            ...
                            <flux:button
                                ...
                                icon="trash"
                                square
                                wire:click="removeStatItem('{{ $block['id'] }}', {{ $i }})"
                                :tooltip="__('Remove stat')"
                            />
                        </div>
                    </div>
                    ...
                    <div class="grid gap-4 p-4 md:grid-cols-2" x-show="open" x-collapse x-cloak>
                        <x-forms.input-translated
                            name="{{ $c }}.items.{{ $i }}.label"
                            :locale="$locale"
                            :multi-locale="$multiLocale"
                            label="{{ __('Label') }}"
                        />
                        <x-forms.input-translated
                            name="{{ $c }}.items.{{ $i }}.value"
                            :locale="$locale"
                            :multi-locale="$multiLocale"
                            label="{{ __('Value') }}"
                        />
        ...
        <div>
            <flux:button
                size="sm"
                icon="plus"
                wire:click="addStatItem('{{ $block['id'] }}')"
            >{{ __('Add stat') }}</flux:button>
        </div>
    </div>
