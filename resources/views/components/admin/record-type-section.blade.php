...
>
    ...
    >
        <div class="flex items-center gap-2 bg-zinc-50 py-1.5 pr-1.5 pl-2 dark:bg-white/5">
            <button
                type="button"
                wire:sort:handle
            ...
            <flux:icon
                :name="$type['icon'] !== '' ? $type['icon'] : 'rectangle-stack'"
            ...
            <flux:button
                ...
                icon="x-mark"
                :tooltip="__('Remove')"
                wire:click="confirmRemove('{{ $type['_key'] }}')"
            />
        </div>
        ...
        <div class="space-y-6 p-4" x-show="open" x-cloak>
            <div class="grid gap-4 sm:grid-cols-2">
                ...
                <flux:field>
                    ...
                    <flux:input
                        wire:model="types.{{ $index }}.slug_prefix"
                        :placeholder="__('e.g. /products/my-item')"
                    />
            ...
            <div class="space-y-3">
                <div class="flex items-center justify-between gap-3">
                    ...
                    <flux:dropdown>
                        ...
                        >{{ __('Add field') }}</flux:button>
                        <flux:menu>
                            @foreach (\App\Enums\FieldType::cases() as $fieldType)
                                <flux:menu.item
                                    :icon="$fieldType->icon()"
                                    wire:click="addField('{{ $type['_key'] }}', '{{ $fieldType->value }}')"
                                >
                                    {{ $fieldType->label() }}</flux:menu.item>
                            @endforeach
                        </flux:menu>
                    </flux:dropdown>
                </div>
                                ...
                                >
                                    <div class="flex items-center gap-2 bg-zinc-50 py-1.5 pr-1.5 pl-2 dark:bg-white/5">
                                        ...
                                        <flux:button
                                            ...
                                            wire:click="removeField('{{ $field['_key'] }}')"
                                        />
                                    </div>
                                    ...
                                    <div class="grid gap-3 p-4 sm:grid-cols-2" x-show="open" x-cloak>
                                        <flux:field>
                                            ...
                                            <flux:input
                                                wire:model="types.{{ $index }}.fields.{{ $fieldIndex }}.label"
                                                x-on:input="title = $event.target.value"
                                            />
                                            ...
                                            <flux:select wire:model.live="types.{{ $index }}.fields.{{ $fieldIndex }}.type">
                                                @foreach (\App\Enums\FieldType::cases() as $fieldType)
                                                    <flux:select.option :value="$fieldType->value">
                                                        {{ $fieldType->label() }}</flux:select.option>
                                                @endforeach
                                            </flux:select>
                                        </flux:field>
                                        <flux:field>
                                        ...
                                        @if (\App\Enums\FieldType::tryFrom($field['type'])?->supportsOptions())
                                            <flux:textarea
                                                wire:model="types.{{ $index }}.fields.{{ $fieldIndex }}.options"
                                                :label="__('Options')"
                                                :description="__('One option per line.')"
                                                class="sm:col-span-2"
                                                rows="3"
                                            />
                                        @endif
                                        <div class="flex flex-wrap items-center gap-x-6 gap-y-3 sm:col-span-2">
                                            <flux:switch
                                                wire:model="types.{{ $index }}.fields.{{ $fieldIndex }}.required"
                                                :label="__('Required')"
                                                align="left"
                                            />
                                            <flux:switch
                                                wire:model="types.{{ $index }}.fields.{{ $fieldIndex }}.translatable"
                                                :label="__('Translatable')"
                                                align="left"
                                            />
                                            <flux:switch
                                                wire:model="types.{{ $index }}.fields.{{ $fieldIndex }}.column"
                                                :label="__('Show in list')"
                                                align="left"
                                            />
                                            <flux:switch
                                                wire:model="types.{{ $index }}.fields.{{ $fieldIndex }}.sortable"
                                                :label="__('Sortable')"
                                                align="left"
                                            />
                                            <flux:switch
                                                wire:model="types.{{ $index }}.fields.{{ $fieldIndex }}.searchable"
                                                :label="__('Searchable')"
                                                align="left"
                                            />
                                        </div>
