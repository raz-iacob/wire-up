    ...
    >
        <div class="flex items-center gap-2 bg-zinc-50 py-1.5 pr-1.5 pl-2 dark:bg-white/5">
            ...
            <flux:button
                ...
                icon="x-mark"
                :tooltip="__('Remove')"
                wire:click="confirmRemove('{{ $menuKey }}', {{ $index }})"
            />
        </div>
        ...
        <div class="grid gap-4 p-4 sm:grid-cols-2" x-show="open" x-cloak>
            @unless ($isAccount)
                <flux:field>
                    ...
                    <flux:input
                        wire:model="{{ $base }}.label"
                        x-on:input="title = $event.target.value"
                        :placeholder="$itemType === 'heading' ? __('e.g. Guides') : null"
                    />
                ...
                @if ($menuKey === 'header' && ($item['appearance'] ?? '') === 'icon')
                    <div class="sm:col-span-2">
                        <flux:field>
                            ...
                            <flux:input
                                wire:model="{{ $base }}.icon_name"
                                placeholder="{{ __('e.g. search, menu, shopping-cart') }}"
                            />
                            <flux:description>
                                {{ __('Any icon name from') }}
                                <flux:link href="https://lucide.dev/icons" external>lucide.dev/icons</flux:link
                                >. {{ __('Brand logos are not included.') }}
                            </flux:description>
