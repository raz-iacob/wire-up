...
>
    <div class="mb-2 flex items-center justify-between gap-3">
        ...
        @else
            ...
            <div class="flex-1">
                <flux:input
                    wire:model.blur="menus.{{ $index }}.name"
                    size="sm"
                    placeholder="{{ __('e.g. Main Menu') }}"
                    class="max-w-xs"
                />
            ...
            <flux:button
                ...
                variant="subtle"
                icon="cog-6-tooth"
                :tooltip="__('Design options')"
                x-on:click="design = ! design"
                x-bind:data-active="design"
            ...
            <flux:button
                ...
                variant="subtle"
                icon="trash"
                :tooltip="__('Delete menu')"
                wire:click="confirmRemoveMenu('{{ $menu['key'] }}')"
            />
        @endif
    </div>

    @unless ($menu['builtin'])
        <div class="mb-4 rounded-lg border border-zinc-200 p-4 dark:border-white/10" x-show="design" x-cloak x-collapse>
            <div class="flex flex-col gap-4">
                ...
                <div class="grid gap-4 sm:grid-cols-2">
                    <flux:switch
                        wire:model="menus.{{ $index }}.display.background"
                        label="{{ __('Background panel') }}"
                        align="left"
                    />
                    <flux:switch
                        wire:model="menus.{{ $index }}.display.sticky"
    ...
    @if ($items !== [])
        <div
            class="space-y-3"
            wire:sort="reorder"
            wire:key="items-{{ $menu['key'] }}-{{ implode('.', array_column($items, '_key')) }}"
        >
    ...
    <div class="mt-6 flex flex-wrap items-center gap-3">
        ...
        @unless ($menu['builtin'])
            <flux:button
                ...
                wire:click="addItem('{{ $menu['key'] }}', 'heading')"
            >
