...
@switch ($layout)
    @case ('card')
        <article class="wire-card flex h-full flex-col overflow-hidden rounded-(--wire-radius) shadow-sm {{ $cardClass }}">
            ...
            <div class="flex grow flex-col gap-3 p-6">
                @if ($member['role'] !== '')
                    <div class="text-xs font-semibold tracking-wide uppercase opacity-60">{{ $member['role'] }}</div>
                @endif
                @if ($member['name'] !== '')
    ...
    @case ('overlay')
        ...
        >
            ...
            <div class="absolute inset-0 bg-gradient-to-t from-black/75 via-black/15 to-transparent transition-opacity duration-300 group-focus-within:opacity-0 group-hover:opacity-0"></div>

            <div class="absolute inset-0 bg-(--wire-primary-bg) opacity-0 transition-opacity duration-300 group-focus-within:opacity-100 group-hover:opacity-100"></div>

            <div class="relative p-6 text-white transition-colors duration-300 group-focus-within:text-(--wire-primary-text) group-hover:text-(--wire-primary-text)">
                @if ($member['name'] !== '')
                ...
                <div class="grid grid-rows-[0fr] opacity-0 transition-all duration-300 group-focus-within:grid-rows-[1fr] group-focus-within:opacity-100 group-hover:grid-rows-[1fr] group-hover:opacity-100">
                    <div class="overflow-hidden">
                        @if ($bio !== '')
                            <div class="mt-3 {{ $bioClass }}">{!! $bio !!}</div>
                        @endif
                        <x-site.blocks.partials.team-socials
        ...
        <article class="flex h-full flex-col">
            @if ($member['photo'])
                <img
                    ...
                    alt="{{ $member['alt'] }}"
                    loading="lazy"
                    class="aspect-square w-full rounded-(--wire-radius) object-cover shadow-sm"
                />
            @endif
            @if ($member['name'] !== '')
