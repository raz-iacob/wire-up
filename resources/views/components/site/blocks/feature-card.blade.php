...
<article
    @class([
        ...
        'rounded-(--wire-radius) p-6 shadow-sm' => $cardStyle,
    ])
...
>
    ...
    @if (strip_tags($item['body']) !== '')
        <div class="grow leading-relaxed opacity-80 [&_a]:underline [&>p]:my-2 *:first:mt-0 *:last:mb-0">{!! $item['body'] !!}</div>
    @endif
    ...
    @if ($item['cta']['enabled'] && $item['cta']['text'] !== '' && $item['cta']['url'] !== null)
        <a
            ...
            class="mt-1 inline-flex items-center justify-center rounded-(--wire-radius) px-5 py-2.5 text-sm font-medium transition hover:opacity-90"
            style="background-color:{{ $item['cta']['bg'] }};color:{{ $item['cta']['fg'] }}"
        >{{ $item['cta']['text'] }}</a>
    @endif
</article>
