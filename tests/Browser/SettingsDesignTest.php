...
it('updates the preview live from a deferred property change without a server round-trip', function (): void {
    ...
    $page->script("window.Livewire.all().find(c => c.\$wire.get('theme') !== undefined).\$wire.set('theme', 'forest', false); void 0");
    $page->wait(0.3);

    $page->assertNoJavascriptErrors()
        ->assertScript("getComputedStyle(document.querySelector('[data-test=preview-body]')).backgroundColor", 'rgb(240, 253, 244)');
});
