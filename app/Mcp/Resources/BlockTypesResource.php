...
final class BlockTypesResource extends Resource
{
    ...
    private function conventions(): array
    {
        return [
            ...
            'heroEffect' => 'The hero block\'s "background.effect" draws an animated layer over whatever background is set, so it combines with an image, a video or a gradient rather than replacing one. "name" accepts "none" or "circuit" — anything else renders nothing. "intensity" is "subtle" or "bold", and "color" takes a CSS colour (hex, rgb(), hsl() or a var()); leave it empty for the theme accent, and note that a value carrying a semicolon or colon is discarded. It stops animating for visitors who ask for reduced motion.',
