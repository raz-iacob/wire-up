...
- The hero block gains an animated background effect. **Circuit board** draws a faded grid, hairline traces routed along it, and charge running down each trace to a contact point that lights up when it arrives. It is drawn in your accent colour, so it follows your theme and flips with dark mode, and it layers over whatever background you already have — an image, a video or a gradient — rather than replacing it. Pick Subtle or Bold, or override the colour. It is markup rather than a video file, so it costs a few kilobytes instead of a few megabytes and stays sharp at any size. Visitors who ask for reduced motion see the board drawn but still.
